...
handlePing(void)
{
	...
	signal(SIGALRM, sig_alrm);
        ...
        if ( (sd = socket(AF_INET, SOCK_RAW, IPPROTO_ICMP)) < 0) { //sock-raw 디테일 해제작업 
	...
	sig_alrm(SIGALRM);
		...
		hlen = iph->ihl * 4;
        	...
        	if (iph->protocol != IPPROTO_ICMP) //프로토콜이 udp인지 tcp인지 icmp인지
                	return;

		if(iph->saddr == sasend.sin_addr.s_addr) { //보낸사람이 맞는지
			icmp = (struct icmp *)(rbuf + hlen);		
			icmplen = len - hlen;

			if (icmp->icmp_type == ICMP_ECHOREPLY) { //echo echo reply 데모에 대한 응답
                		if (icmp->icmp_id != pid) //내껀지 확인 
                        		return;

				gettimeofday(&tvrecv, NULL);
                		tvsend = (struct timeval *) icmp->icmp_data; //tvsend 보낸시간 
...
send_msg(void)
{
	...
	int	datalen = 56;
        ...
        icmp->icmp_code = 0;
        icmp->icmp_id = pid;
        icmp->icmp_seq = nsent++; //시간 
        memset(icmp->icmp_data, 0xa5, datalen);

        gettimeofday( (struct timeval *) icmp->icmp_data, NULL ); //gettimeofday 시간너흔 함수(메시지)
