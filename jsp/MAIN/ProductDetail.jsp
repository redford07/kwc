...
<%--  	<img src="c:/data/<%=product.getImage()%>" > --%>	
		...
		<td width="30%" valign="top">
			...
			<tr>
			<td><b>날    짜 : </b><%=product.getDate()%></td>
			</tr>
			<tr>
			<td><b>수    량 : </b><%=product.getStock()%></td>
			</tr>
			</table>
		
		</td>
		<td width="50%" valign="top"><b>내    용</b><br><pre><%=product.getDetail()%></pre></td>
		</tr>
		<tr> 
		...
		<a href="javascript:productCart('<%=product.getNo()%>')">찜하기</a>
		<input type="button" value="판매자연락처"
									onClick="productseller('<%=product.getUserid()%>')">
		<%} else if(!(id.equals(chna))){%>
		<a href="javascript:productCart('<%=product.getNo()%>')">찜하기</a>
		<input type="button" value="판매자연락처"
									onClick="productseller('<%=product.getUserid()%>')">
		<%} else{%>
	...
	<form name="cart" method="post" action="ProductProc.jsp?flag=cart">
	<input type=hidden name=no>
	</form>
